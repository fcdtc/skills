...
### Step 0: Check Raw Source Preference

Before starting research, determine whether the user wants to keep raw source materials:

- If the user explicitly asks to **keep raw sources / 保留一手资料 / save original pages**, set `keep_raw_sources = true`.
- If the user says **no raw sources / don't save originals**, set `keep_raw_sources = false`.
- If not specified, **ask the user**: "Do you want to save the raw web page content as local markdown files for reference? (保留一手资料？)"

When `keep_raw_sources = true`:
- Create a `research_[topic_name]/raw_sources/` subfolder.
- Every time you or a subagent fetches a web page (via `WebFetch` or similar), save the full fetched content to `research_[topic_name]/raw_sources/source_[N]_[short_label].md` with this format:
  ```markdown
  # [Page Title or URL]
  - **URL:** [original URL]
  - **Fetched:** [date]
  ...
  ```
- Instruct subagents to do the same when `keep_raw_sources = true` — include this in their task prompt:
  ```
  IMPORTANT: Save the raw content of every web page you fetch to
  research_[topic_name]/raw_sources/source_[N]_[short_label].md
  using the format: title, URL, date, then full content.
  ```
- In the final report, add a "Raw Sources Index" section listing all saved raw source files with their URLs.
...
1. **Create a research folder** - Organize all research files in a dedicated folder relative to the current working directory:
   ...
   ```
   If `keep_raw_sources = true`, also create `research_[topic_name]/raw_sources/`.
